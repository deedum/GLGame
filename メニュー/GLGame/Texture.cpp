...
//		MessageBox(NULL, _T("テクスチャ読み込みに失敗しました"), _T("error"), MB_OK | MB_ICONSTOP);
		return 0;
	}
	...
	if (pTextureImage->type != 3 && pTextureImage->type != 4) {
		...
		free(pTextureImage);
//		MessageBeep(MB_ICONSTOP);
//		MessageBox(NULL, _T("この形式はサポートしていません"), _T("error"), MB_OK | MB_ICONSTOP);
		return 0;
	}
...
void ReleaseTexture(GLuint uTexID)
{
	if (uTexID == 0)
		return;
	glDeleteTextures(1, &uTexID);	// テクスチャ オブジェクトを解放
}
